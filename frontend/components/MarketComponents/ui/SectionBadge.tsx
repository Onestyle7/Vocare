...
const toneClasses: Record<Tone, string> = {
  neutral: 'border-white/50 text-white/90 bg-white/5 dark:border-white/20 dark:text-white/90',
  violet: 'border-[#915EFF]/50 text-[#915EFF] bg-[#915EFF]/10 dark:border-[#915EFF]/40',
  ...
  return (
    <div
      className={`font-korbin mt-1 mb-6 inline-flex h-[38px] min-w-[160px] items-center ${
        center ? 'justify-center' : 'justify-start pr-4 pl-3'
      } gap-2 rounded-full border text-sm backdrop-blur ${toneClasses[tone]} ${className}`}
      role="status"
    >
      <span aria-hidden className="inline-block h-2 w-2 rounded-full bg-current opacity-80" />
