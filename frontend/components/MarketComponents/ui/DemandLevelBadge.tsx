...
function mapLevel(level: string): { key: 'very-high' | 'high' | 'medium' | 'low'; label: string } {
  ...
  if (['low', 'niski', 'niska'].some((k) => v.includes(k))) return { key: 'low', label: 'Low' };
...
const DemandLevelBadge: React.FC<DemandLevelBadgeProps> = ({
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 rounded-full font-medium ${toneClass[mapped.key]} ${sizes[size]} ${className}`}
    >
      {withDot && <span aria-hidden className="h-1.5 w-1.5 rounded-full bg-current" />}
      {mapped.label}
    </span>
  );
};
